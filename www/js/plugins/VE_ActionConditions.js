...
(function () {
  ...
  PluginManager.requiredPlugin = function (name, required, version) {
    if (!VictorEngine.BasicModule) {
      ...
      VictorEngine.ActionConditions.requiredPlugin.call(
        this,
        name,
        required,
        version
...
(function () {
  ...
  VictorEngine.ActionConditions.loadNotetagsValues =
  ...
  VictorEngine.loadNotetagsValues = function (data, index) {
    ...
    if (this.objectSelection(index, list))
      VictorEngine.ActionConditions.loadNotes(data);
  };
  ...
  VictorEngine.ActionConditions.meetsUsableItemConditions =
    Game_BattlerBase.prototype.meetsUsableItemConditions;
  Game_BattlerBase.prototype.meetsUsableItemConditions = function (item) {
    if (!this.meetCustomActionConditions(item)) return false;
    return VictorEngine.ActionConditions.meetsUsableItemConditions.call(
      this,
      item
    );
  };
  ...
  Game_BattlerBase.prototype.actionConditionParam = function (condition) {
    if (condition.type === "switch") {
      return (
        $gameSwitches.value(condition.value) === (condition.check === "on")
      );
    } else if (condition.type === "state") {
      return (
        this.isStateAffected(condition.value) === (condition.check === "added")
      );
    } else {
      if (condition.rate) {
        var rate = "Rate() * 100";
      ...
      if (condition.check === "higher" && eval(result) < condition.value)
        return true;
      if (condition.check === "lower" && eval(result) > condition.value)
        return true;
      if (condition.check === "equal" && eval(result) !== condition.value)
        return true;
      if (condition.check === "different" && eval(result) === condition.value)
        return true;
    }
    return false;
  };
