...
(function () {
  ...
  const _SID_ARMSGCLR =
    +_PARAMETERS["既読メッセージ文字色制御用スイッチID"] || 0;
  const _TAG_ORGMSG = _PARAMETERS["修正前文章判定用タグ"] || "";

  function _(f) {
    return (f[_PRODUCT] = f[_PRODUCT] || {});
  }
  ...
  function isSkip() {
    var skip = false;
    if (
      Input.isPressed(_KEY_MSGSKIP) &&
      ($gameSwitches.value(_SID_URSKIP) || _($gameMessage).already_read)
    ) {
      skip = true;
    } else if (
      Input.isPressed(_KEY_ARMSGSKIP) &&
      _($gameMessage).already_read
    ) {
      skip = true;
    }
    return skip;
  ...
  Scene_Map.prototype.isFastForward = function () {
    return (
      _Scene_Map_isFastForward.call(this) ||
      ($gameMap.isEventRunning() && !SceneManager.isSceneChanging() && isSkip())
    );
  };
  ...
  Window_Message.prototype.normalColor = function () {
    ...
    return $gameSwitches.value(_SID_ARMSGCLR) && _($gameMessage).already_read
      ? this.textColor(_CID_ARMSG)
      : color;
  };
})();
