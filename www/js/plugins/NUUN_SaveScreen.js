 ...
 * info.AnyName:表示文字列
 * info.title:ゲームタイトル名
 * info.destinationName:行動目標(要)
 *
 * NUUN_Chapterと併用した場合、プラグインパラメータで設定したファイル名横文字列表示変数番号及び、
 * プラグインコマンドのファイル名横表示文字列で設定している場合は、後者の設定が適用されます。
 *
 ...
 * @param AnyNameMode
 * @text drawTextEx表示
 * @desc テキストをdrawTextEx(制御文字使用可能)で表示します。OFFでdrawText(制御文字不可)で表示。
 * @type boolean
 * @default false
 * @parent AnyName
 *
 ...
 * @text ファイル名横文字列表示評価式
 * @type combo
 * @option info.AnyName ? info.AnyName : info.title
 * @option info.destinationName;//要NUUN_Destination 行動目標
 ...
 * @option 'info.orgParam? info.orgParam[0] : 0'//数値表示など
 * @option info.destinationName;//要NUUN_Destination 行動目標
 * @default
 *
 * @param X_Position
...
(() => {
  ...
  const ContentsBackVisible = eval(
    parameters["ContentsBackVisible"] || "false"
  );
  const AutomaticSetting = eval(parameters["AutomaticSetting"] || "true");
  const AnyNameMode = eval(parameters["AnyNameMode"] || "false");
  ...
  DataManager.makeSavefileInfo = function () {
    ...
    const anyName = $gameSystem.saveAnyName
      ? $gameSystem.saveAnyName
      : $gameVariables.value(AnyNameVariable);
    if (anyName) {
      info.AnyName = anyName;
    } else if (Imported.NUUN_Chapter) {
  ...
  Window_SavefileList.prototype.drawItemBackground = function (index) {
    ...
    const buckgroundImg =
      info && !!info.contentsBuck ? info.contentsBuck : ContentsBackGroundImg;
    if (buckgroundImg) {
  ...
  Window_SavefileList.prototype.drawTitle = function (savefileId, x, y) {
    if (this._FaceOn) {
      ...
      _Window_SavefileList_drawTitle.call(
        this,
        savefileId,
        x + FailNameX,
        y + FailNameY
  ...
  Window_SavefileList.prototype.drawAnyName = function (info, x, y, width) {
    ...
    if (anyName) {
      ...
      if (AnyNameMode) {
        this.drawTextEx(anyName, x, y, width);
      } else {
