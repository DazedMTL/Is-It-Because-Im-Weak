...
(function () {
  ...
  PluginManager.requiredPlugin = function (name, required, version) {
    if (!VictorEngine.BasicModule) {
      ...
      VictorEngine.DamgePopup.requiredPlugin.call(
        this,
        name,
        required,
        version
...
(function () {
  ...
  if (Imported["VE - Basic Module"]) {
    ...
    VictorEngine.Parameters.DamgePopup.HeightAdjust = eval(
      parameters["Height Adjust"]
    );
    VictorEngine.Parameters.DamgePopup.OverlapAdjust =
      Number(parameters["Overlap Adjust"]) || 0;
    VictorEngine.Parameters.DamgePopup.DelayAdjust =
      Number(parameters["Delay Adjust"]) || 0;
    VictorEngine.Parameters.DamgePopup.Wait = String(parameters["Wait"]).trim();
    VictorEngine.Parameters.DamgePopup.Up = String(
      parameters["Move Up"]
    ).trim();
    VictorEngine.Parameters.DamgePopup.Down = String(
      parameters["Move Down"]
    ).trim();
    VictorEngine.Parameters.DamgePopup.Left = String(
      parameters["Move Left"]
    ).trim();
    VictorEngine.Parameters.DamgePopup.Right = String(
      parameters["Move Right"]
    ).trim();
    VictorEngine.Parameters.DamgePopup.Zoom = String(parameters["Zoom"]).trim();
    VictorEngine.Parameters.DamgePopup.Stretch = String(
      parameters["Stretch"]
    ).trim();
    VictorEngine.Parameters.DamgePopup.Melt = String(parameters["Melt"]).trim();
    VictorEngine.Parameters.DamgePopup.PopHigh = String(
      parameters["Pop High"]
    ).trim();
    VictorEngine.Parameters.DamgePopup.PopLow = String(
      parameters["Pop Low"]
    ).trim();
    VictorEngine.Parameters.DamgePopup.FallHigh = String(
      parameters["Fall High"]
    ).trim();
    VictorEngine.Parameters.DamgePopup.FallLow = String(
      parameters["Fall Low"]
    ).trim();
    var list = [
      "HP Damage",
    ...
    for (var i = 0; i < list.length; i++) {
      var type = list[i];
      VictorEngine.Parameters.DamgePopup[type + " Text"] = String(
        parameters[type + " Text"]
      ).trim();
      VictorEngine.Parameters.DamgePopup[type + " Fontface"] = String(
        parameters[type + " Fontface"]
      ).trim();
      VictorEngine.Parameters.DamgePopup[type + " Fontsize"] = String(
        parameters[type + " Fontsize"]
      ).trim();
      VictorEngine.Parameters.DamgePopup[type + " Color"] = String(
        parameters[type + " Color"]
      ).trim();
      VictorEngine.Parameters.DamgePopup[type + " Motion"] = String(
        parameters[type + " Motion"]
      ).trim();
      VictorEngine.Parameters.DamgePopup[type + " Flash"] = String(
        parameters[type + " Flash Color"]
      ).trim();
      VictorEngine.Parameters.DamgePopup[type + " Delay"] =
        Number(parameters[type + " Delay"]) || 0;
      VictorEngine.Parameters.DamgePopup[type + " Offset X"] =
        Number(parameters[type + " Offset X"]) || 0;
      VictorEngine.Parameters.DamgePopup[type + " Offset Y"] =
        Number(parameters[type + " Offset Y"]) || 0;
      VictorEngine.Parameters.DamgePopup[type + " Roll"] = eval(
        parameters[type + " Roll"] || "false"
      );
    }
    var list = [
      "Wait",
    ...
    for (var i = 0; i < list.length; i++) {
      var type = list[i];
      VictorEngine.Parameters.DamgePopup[type] = String(
        parameters[type]
      ).trim();
    }
    for (var i = 1; i < 21; i++) {
      var type = list[i];
      VictorEngine.Parameters.DamgePopup["Custom " + String(i)] = String(
        parameters["Custom " + String(i)]
      ).trim();
    }
  }
  ...
  VictorEngine.DamgePopup.processMotions = function () {
    ...
    for (var i = 0; i < list.length; i++) {
      this.motions[list[i].toLowerCase()] = this.setupMotions(
        parameters[list[i]]
      );
    }
    for (var i = 1; i < 21; i++) {
      this.motions["custom " + String(i)] = this.setupMotions(
        parameters["Custom " + String(i)]
      );
    }
  };
  ...
  VictorEngine.DamgePopup.invokeCounterAttack =
  ...
  VictorEngine.DamgePopup.invokeMagicReflection =
  ...
  VictorEngine.DamgePopup.calcElementRate =
  ...
  Game_Battler.prototype.callCustomPopup = function (
    text,
    motion,
    font,
    size,
  ...
  Sprite_Battler.prototype.damagePopupSprite = function (type, value) {
    ...
    var overlap =
      VictorEngine.Parameters.DamgePopup.OverlapAdjust * this._damageSprites;
    var adjust = VictorEngine.Parameters.DamgePopup.HeightAdjust
      ? this.center().y
      : 8;
...
(function () {
  ...
  Window_DamageSprite.prototype.processDrawIcon = function (
    iconIndex,
    textState
  ) {
...
(function () {
  ...
  Sprite_CustomDamage.prototype.setupFlashEffect = function () {
    ...
    if (resist && sprite) {
      ...
      if (hex) {
        var color = VictorEngine.hexToRgb(
          eval(sprite.window.getFontColor(hex))
        );
        if (color) {
          color.push(160);
  ...
  Sprite_CustomDamage.prototype.popupMotion = function (type) {
    ...
    return type === "Custom"
      ? this._value
      : VictorEngine.DamgePopup.popup[type];
  };
  ...
  Sprite_CustomDamage.prototype.createDigits = function (type) {
    ...
    for (var i = 0; i < string.length; i++) {
      ...
      sprite.digit = n;
      sprite.delay =
        delay < 0 ? Math.abs(delay) * (string.length - i) : delay * i;
  ...
  Sprite_CustomDamage.prototype.setupDigits = function (sprite, i) {
    ...
    var n = roll
      ? (this._random[i] + Math.randomInt(9) + 1) % 10
      : sprite.digit;
  ...
  Sprite_CustomDamage.prototype.updateDigits = function () {
    ...
    for (var i = 0; i < this._digits.length; i++) {
      ...
      if (sprite.setup.roll && sprite.text.duration % 4 === 0)
        this.setupDigits(sprite, i);
      if (sprite.text.duration === 0) this.refreshMotion(sprite);
    }
  };
