...
(function () {
    ...
    for (var i = 1; i < $dataStates.length; i++) {
      if (Number($dataStates[i].meta.RSGgroupId) === groupId) {
        ...
        if (isFinite(Number($dataStates[i].meta.RSGxRate))) {
          states[$dataStates[i].id].xRate = Number(
            $dataStates[i].meta.RSGxRate
          );
        }
        if (isFinite(Number($dataStates[i].meta.RSGsRate))) {
          states[$dataStates[i].id].sRate = Number(
            $dataStates[i].meta.RSGsRate
          );
        }
      }
    }
      ...
      case "rankUp":
        for (var i = 0; i < ids.length; i++) {
          ...
          for (var j = 0; j < ids.length; j++) {
            your = states[ids[j]];
            if (ids[i] != ids[j] && which * (your.rank - my.rank) > 0) {
          ...
          for (var k = 0; k < ids.length; k++) {
            your = states[ids[k]];
            if (
              which * (your.rank - my.rank) ===
              Math.min.apply(this, defferences)
            ) {
      ...
      case "state":
        for (var n = 0; n < states.length; n++) {
          if (isFinite(Number($dataStates[ids[n]].meta.RSGtargetState))) {
            states[ids[n]].target = Number(
              $dataStates[ids[n]].meta.RSGtargetState
            );
          }
        }
        break;
  ...
  RSGM.targetOf = function (stateId) {
    if (this.isRSG(stateId)) {
      ...
      if (target === -1) {
        return Number(
          Object.keys(states)[Math.floor(Math.random() * Object.keys(states))]
        );
      } else {
        return target;
      }
  ...
  RSGM.isLimited = function (stateId) {
    if (
      this.groupOf(stateId).sFlag === "param" ||
  ...
  RSGM.limitRangeOf = function (stateId) {
    ...
    var states = this.groupOf(stateId).states;
    var condition =
      this.groupOf(stateId).rankType + "_" + this.groupOf(stateId).target;
  ...
  RSGM.matchStateToLimitOf = function (stateId, value) {
    ...
    switch (this.whichSideOfLimit(stateId, value)) {
      ...
      case "high":
      case "low":
        if (group.target === "state" || group.target === "none") {
          return this.targetOf(stateId);
        } else if (group.target === "rankUp" || group.target === "rankDown") {
          var matches = [];
          var others = this.othersOf(stateId);
          for (var i = 0; i < others.length; i++) {
  ...
  //代表ステートの付加(処理)
  Game_Action.prototype.RSG_addPrimitiveState = function (
    target,
    groupId,
    element
  ) {
    ...
    if (this.item().damage.elementId < 0) {
      ...
      for (var i = 1; i < RSGM.database.length; i++) {
        for (var j = 0; j < elements.length; j++) {
          this.RSG_addPrimitiveState(
            target,
            RSGM.database[i].groupId,
            elements[j]
          );
        }
      }
  ...
  Game_Battler.prototype.RSG_flag_turns = function (timing) {
    ...
    for (var i = 0; i < states.length; i++) {
      if (
        RSGM.groupOf(states[i]).flag === "turns" &&
  ...
  Game_Battler.prototype.RSG_sFlag_addedState = function (stateId) {
    if (
      this._result.addedStates.some(function (addedStateId) {
      ...
      for (var i = 0; i < states.length; i++) {
        var group = RSGM.groupOf(states[i]);
        if (group.sFlag === "addedState" && group.sValue === stateId) {
          this.RSG_rateToChange(states[i], group.states[states[i]].sRate);
        } else if (
          group.sFlag === "addedStateGroup" &&
  ...
  Game_Battler.prototype.RSG_sFlag_usingSkill = function (item) {
    if (DataManager.isSkill(item)) {
      ...
      for (var i = 0; i < states.length; i++) {
        var group = RSGM.groupOf(states[i]);
        if (group.sFlag === "usingSkill" && group.sValue === item.id) {
          this.RSG_rateToChange(states[i], group.states[states[i]].sRate);
        } else if (
  ...
  Game_Battler.prototype.RSG_matchState = function (stateId) {
    if (RSGM.isRSG(stateId) && this.RSG_hasStrongersOf(stateId)) {
      stateId = 0;
    }
  ...
  RRT.StateGroups._Game_Battler_onBattleEnd =
    Game_Battler.prototype.onBattleEnd;
  ...
  RRT.StateGroups._Game_Battler_removeStatesAuto =
    Game_Battler.prototype.removeStatesAuto;
  ...
  RRT.StateGroups._Game_Actor_updateStateSteps =
    Game_Actor.prototype.updateStateSteps;
  ...
  RRT.StateGroups._Game_Battler_removeState =
    Game_Battler.prototype.removeState;
  ...
  RRT.StateGroups._Game_Battler_onBattleStart =
    Game_Battler.prototype.onBattleStart;
  ...
  RRT.StateGroups._Game_Battler_onAllActionsEnd =
    Game_Battler.prototype.onAllActionsEnd;
  ...
  RRT.StateGroups._Game_Action_executeHpDamage =
    Game_Action.prototype.executeHpDamage;
  ...
  //デバッグ用に置いておきます。
  var _RSG_Game_Interpreter_pluginCommand =
