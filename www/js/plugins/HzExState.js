...
(function () {
  ...
  Game_Battler.prototype.addState = function (stateId) {
    ...
    if (added) {
      ...
      if (stackStates.length !== 0) {
        ...
        var nextStates = [];
        // ステート付与のメッセージを削除
        this._result.addedStates = this._result.addedStates.filter(function (
          elm
        ) {
          return elm !== stateId;
        });
        ...
        stackStates.forEach(function (id) {
          ...
          // ステート解除のメッセージを削除
          self._result.removedStates = self._result.removedStates.filter(
            function (elm) {
              return elm !== id;
            }
          );
          ...
          if (stateId === nextStateId) {
            // ステート付与のメッセージを削除
            self._result.addedStates = self._result.addedStates.filter(
              function (elm) {
                return elm !== stateId;
              }
            );
        ...
        // ステート解除のメッセージを削除
        this._result.removedStates = this._result.removedStates.filter(
          function (elm) {
            return elm !== stateId;
          }
        );
  ...
  // アイテム・スキルによるステート解除
  var _Game_Action_itemEffectRemoveState =
  ...
  // ダメージによるステートの自動解除
  var _Game_Battler_removeStatesByDamage =
  ...
  Game_Action.prototype.testItemEffect = function (target, effect) {
    ...
    if (effect.code === Game_Action.EFFECT_ADD_STATE) {
      ...
      if ($dataStates[effect.dataId].meta.hzExState_stack !== undefined) {
        var stackParms =
          $dataStates[effect.dataId].meta.hzExState_stack.split(",");
