...
(function () {
  ...
  VictorEngine.Parameters.BasicModule.hit = String(
    parameters["Hit Rate Name"]
  ).trim();
  VictorEngine.Parameters.BasicModule.eva = String(
    parameters["Evasion Rate Name"]
  ).trim();
  VictorEngine.Parameters.BasicModule.cri = String(
    parameters["Critical Rate Name"]
  ).trim();
  VictorEngine.Parameters.BasicModule.cev = String(
    parameters["Critical Evasion Name"]
  ).trim();
  VictorEngine.Parameters.BasicModule.hev = String(
    parameters["Magic Evasion Name"]
  ).trim();
  VictorEngine.Parameters.BasicModule.mrf = String(
    parameters["Magic Reflection Name"]
  ).trim();
  VictorEngine.Parameters.BasicModule.cnt = String(
    parameters["Counter Attack Name"]
  ).trim();
  VictorEngine.Parameters.BasicModule.hrg = String(
    parameters["HP Regeneration Name"]
  ).trim();
  VictorEngine.Parameters.BasicModule.mrg = String(
    parameters["MP Regeneration Name"]
  ).trim();
  VictorEngine.Parameters.BasicModule.trg = String(
    parameters["TP Regeneration Name"]
  ).trim();
  VictorEngine.Parameters.BasicModule.tgr = String(
    parameters["Target Rate Name"]
  ).trim();
  VictorEngine.Parameters.BasicModule.grd = String(
    parameters["Guard Rate Name"]
  ).trim();
  VictorEngine.Parameters.BasicModule.rec = String(
    parameters["Recovery Effect Name"]
  ).trim();
  VictorEngine.Parameters.BasicModule.pha = String(
    parameters["Pharmacology Name"]
  ).trim();
  VictorEngine.Parameters.BasicModule.mcr = String(
    parameters["MP Cost Rate Name"]
  ).trim();
  VictorEngine.Parameters.BasicModule.tcr = String(
    parameters["TP Charge Rate Name"]
  ).trim();
  VictorEngine.Parameters.BasicModule.pdr = String(
    parameters["Physical Damage Name"]
  ).trim();
  VictorEngine.Parameters.BasicModule.mdr = String(
    parameters["Magical Damage Name"]
  ).trim();
  VictorEngine.Parameters.BasicModule.fdr = String(
    parameters["Floor Damage"]
  ).trim();
  VictorEngine.Parameters.BasicModule.ext = String(
  ...
  VictorEngine.BasicModule.checkPlugins = function (name, req, ver) {
    ...
    if (ver && req && (!Imported[req] || Number(Imported[req]) < Number(ver))) {
      ...
      req &&
      Imported["VE - Basic Module"] &&
      !this.loadedPlugins["VE - Basic Module"]
    ) {
  ...
  VictorEngine.battlerIndex = function (battler) {
    if (
      battler &&
      battler.isActor() &&
      $gameParty.members().contains(battler)
    ) {
      return 1 + $gameParty.members().indexOf(battler);
    } else if (
      battler &&
      battler.isEnemy() &&
      $gameTroop.members().contains(battler)
    ) {
      return (
        1 +
        $gameParty.maxBattleMembers() +
        $gameTroop.members().indexOf(battler)
      );
    } else {
      return 0;
    }
  ...
  BattleManager.startCounterAction = function (
    subject,
    target,
    targets,
    action
  ...
  BattleManager.averageTargetsPositions = function (targets) {
    var x =
      targets.reduce(function (r, member) {
  ...
  Game_Unit.prototype.averageX = function () {
    return (
      this.aliveMembers().reduce(function (r, member) {
        return r + member.screenX();
  ...
  Game_BattlerBase.prototype.turnTowardOpponents = function () {
    this.turnTowardPosition(
      this.opponents().averageX(),
      this.opponents().averageY()
    );
  };

  Game_BattlerBase.prototype.turnAwayFromOpponents = function () {
    this.turnAwayFromPosition(
      this.opponents().averageX(),
      this.opponents().averageY
    );
  };
  ...
  Game_BattlerBase.prototype.turnTowardPosition = function (
    positionX,
    positionY
  ) {
  ...
  Game_BattlerBase.prototype.turnAwayFromPosition = function (
    positionX,
    positionY
  ) {
    if (this.screenX() < positionX) {
  ...
  Game_BattlerBase.prototype.isBehind = function (target) {
    return (
      (target.isFacingRight() && this.screenX() < target.screenX()) ||
  ...
  Game_Enemy.prototype.battlerDirection = function () {
    return (
      this._battlerDirection || ($gameSystem.isSideView() ? "right" : "left")
    );
  };
  ...
  Sprite_Actor.prototype.updateBattlerDirection = function () {
    if (this._battler && !this.isDisabledMotion()) {
      if (
        (this._battler.isFacingLeft() && this.scale.x < 0) ||
  ...
  Window_BattleLog.prototype.waitForBattleAnimation = function (
    animationId,
    speed
  ) {
  ...
  Window_BattleLog.prototype.startCounterAction = function (
    subject,
    action,
    targets
  ) {
  ...
  Window_BattleLog.prototype.setupStartAction = function (
    subject,
    action,
    targets
  ) {
  ...
  Window_BattleLog.prototype.prepareAction = function (
    subject,
    action,
    targets
  ) {
  ...
  Window_BattleLog.prototype.prepareUniqueAction = function (
    subject,
    action,
    target,
    repeat
  ...
  Window_BattleLog.prototype.prepareUniqueActionStep1 = function (
    subject,
    action,
    target,
    repeat
  ...
  Window_BattleLog.prototype.prepareUniqueActionStep2 = function (
    subject,
    action,
    target,
    repeat
  ...
  Window_BattleLog.prototype.prepareUniqueActionStep3 = function (
    subject,
    action,
    target,
    repeat
  ...
  Window_BattleLog.prototype.uniqueActionAnimationId = function (
    subject,
    item
  ) {
  ...
  Window_BattleLog.prototype.restoreMethodsStack = function (index) {
    this._methodsStack[index] = this._methodsStack[index].concat(
      this._saveMethodsStack
    );
  ...
  Window_BattleLog.prototype.updateStackAction = function (
    index,
    subject,
    target
  ) {
  ...
  Window_BattleLog.prototype.stackWaitFoAnimation = function (
    index,
    animationId,
    speed
  ) {
    var speed = Math.max(speed, 1) || 1;
    this.stackWaitForTime(
      index,
      VictorEngine.waitAnimation(animationId) / speed
    );
  };
  ...
  Window_BattleLog.prototype.updateStackWaitMode = function (index) {
    ...
    if (waitMode.contains("effect")) {
      var waiting = battler
        ? battler.isSpriteEffecting()
        : this._spriteset.isEffecting();
      if (waiting) {
        return true;
      } else {
  ...
  Window_BattleLog.prototype.setupCurrentAction = function (
    subject,
    action,
    targets
  ) {};
